...
local function SuperCheck(mo)
    local p = mo.player

    if p.powers[pw_super] or (p.solchar and p.solchar.istransformed) then return true end
    return false
end
...
local function SuperCoronaThink(mo)
    local t = mo.target
    if not (t and t.player and SuperCheck(t)) then P_KillMobj(mo) return end
